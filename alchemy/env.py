...
class Env:
    ...
    def start(self, task, callback):
        finished, test_scores = self.evaluate(task)
        if finished:
            return test_scores

        for _ in range(self.limit):
            self.step(task, self.mcts_rounds)

            finished, test_scores = self.evaluate(task) if 0 == len(self.scores) % self.eval_delay else (False, None)
    ...
    def step(self, task, mcts_rounds):
    ...
    def evaluate(self, task):
        ...
        for _ in range(self.eval_limit):
            ...
            succ += int(done)
            if not task.goal_met(max(np.mean(scores, 1))):
...
#            if action.shape[1] == 3: print("-->",steps, self.max_n_episode, learn_mode)
        ...
        for i, s in enumerate(state):
            history[i].append(s.view(-1).cpu().numpy())
        state = np.asarray(history).reshape(len(history), -1)
        state = torch.from_numpy(state)#.view(len(history), -1)
        ...
        yield (
                ...
                torch.zeros(action.shape), # will not be used
                torch.zeros(reward.shape), # will not be used
    ...
    def _simulate(self, task, seeds):
        ...
        while len(scores) != len(seeds):
            ...
            last = 0
            for data in self._learning_loop(task, f_pi, history, seed, True):
                self.step_count += 1
...
# just wrapper
    ...
    def _print_stats(self, e, rewards, action):
        print("\r[{:5d}>{:6d}::{:2d}] steps = {:4d}, max_step = {:3d}/{:3d}, reward={:2f} <action={}>{}".format(
            ...
            self.score, action.view(self.total_envs, -1)[0].view(-1), " "*20), end="")
