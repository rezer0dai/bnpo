...
class NoisyLinear(nn.Linear):
    def __init__(self, in_features, out_features, device):
        ...
        self.register_buffer('noise', torch.zeros(out_features, in_features).to(device))

        self.device = device
        self.apply(initialize_weights)

    def forward(self, sigma, data):
        return F.linear(data, self.weight + sigma * Variable(self.noise).to(data.device))
...
class NoisyNet(nn.Module):
    def __init__(self, layers):
        ...
        for i, layer in enumerate(self.layers):
            self.sigma.append(
                nn.Parameter(torch.Tensor(layer.out_features, layer.in_features).fill_(.017).to(layer.device)))
