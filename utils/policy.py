...
class PPOHead(nn.Module):
    def __init__(self, action_size, f_clip):
        super().__init__()
        self.f_clip = f_clip
        ...
        if loss.grad_fn is None:
            ...
            if vg_c != self.vg_c and 0 == self.vg_c % 20:
                print("PPO too off, from sampled actions, policies problems!! ",
                        self.vg_c, self.po_c, new_probs.mean(), old_probs.mean())
...
class GAE:
    ...
    def gae(self, n_steps, rewards, values):
        ...
        values = values.cpu().numpy() # speedup A LOT!!
        return [ torch.from_numpy(self._gae(
            values[i+n], values[i:i+n], rewards[i:i+n]
            ) if n > 0 else values[0] * 0.) for i, n in enumerate(n_steps) ]
